...
public class ItemRandomThing extends ItemNAT {
    ...
    public float getStrVsBlock(ItemStack stack, Block block) {
        if(block instanceof BlockNAT)
            return NBTHelper.getInt(stack, "powar", 1);
        return NBTHelper.getInt(stack, "powar", 1) / 8.0f;
    }

    @Override
    public boolean onEntitySwing(EntityLivingBase entityLiving, ItemStack stack) {
        entityLiving.attackEntityFrom(DamageSource.generic, NBTHelper.getInt(stack, "powar", 1) / 8.0f);
        return false;
    }

    @Override
    public boolean onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ) {
        int range = NBTHelper.getInt(stack, "powar", 1) / 8 + 1;
        int numGoes = 0;
        outerLoop:
    ...
    private boolean removeItem(ItemStack inUse, EntityPlayer player) {
        ...
        for(int i = 0; i < mainInventory.length; i++) {
            ...
            if(itemStack != null && !(itemStack == inUse) && itemStack.getItem() instanceof ItemRandomThing && itemStack.stackSize > 0) {
                if(NBTHelper.getInt(itemStack, "powar", 1) <= 1) {
                    itemStack.stackSize--;
                    if(itemStack.stackSize <= 0)
                        mainInventory[i] = null;
                }
                else
                    NBTHelper.setInt(itemStack, "powar", NBTHelper.getInt(itemStack, "powar", 1) - 1);
                return true;
            }
        }
        return false;
    ...
    public void addInformation(ItemStack stack, EntityPlayer playerIn, List tooltip, boolean advanced) {
        ...
        tooltip.add("power: " + NBTHelper.getInt(stack, "powar", 1));
    }

    @Override
    public ItemStack onItemRightClick(ItemStack itemStackIn, World worldIn, EntityPlayer playerIn) {
        playerIn.heal(NBTHelper.getInt(itemStackIn, "powar", 1) / 8.0f);
    ...
    public void onCreated(ItemStack stack, World worldIn, EntityPlayer playerIn) {
        if(!NBTHelper.hasTag(stack, "powar"))
            NBTHelper.setInt(stack, "powar", 1);
    }
}
